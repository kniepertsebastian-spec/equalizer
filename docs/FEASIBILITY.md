...
Der Button „Probe session 0 as root“ startet über `su` einen kurzlebigen
`app_process`-Prozess aus dem installierten APK. Der Prozess prüft seine UID
und versucht, einen `Equalizer` auf Session `0` zu konstruieren. Bei diesem
Probe wird der Effekt nicht aktiviert und sofort freigegeben.

Der zusätzliche „Start root output EQ“-Test hält einen separaten Root-Prozess
am Leben, aktiviert den Output-Mix-Equalizer und sendet Bandänderungen über
stdin/stdout. `Stop` gibt den Effekt wieder frei. Damit wird erstmals die
Root-IPC-Kette samt tatsächlichem Aktivieren und Parameter-Setzen ausprobiert.
Das ist weiterhin ein experimentelles, geräteabhängiges Backend: Es hängt
davon ab, ob die Geräteimplementierung `Equalizer` auf Session `0` unter UID
0 zulässt und den Effekt auf den gemischten Ausgang routet.
...
4. Nach einem erfolgreichen Probe den Live-Test starten, während Musik aus
   einer anderen App läuft. Eine Band absenken/anheben, den hörbaren Effekt
   prüfen und anschließend `Stop` drücken.
5. Status und Ergebnis samt Geräte-/Android-Version zurückmelden.

Erwartete Probe-Statusfälle sind `ROOT NOT CONFIRMED`,
`ROOT OK, ATTACH FAILED` oder `ROOT + ATTACH OK`. Nur der separate Live-Test
aktiviert den Output-Mix-Effekt; er ist klar als systemweit potenziell hörbar
beschriftet und muss danach mit `Stop` beendet werden.
