...
- ✅ Punkt 7 (Re-Attach-Implementierung, 25. September 2026): Begrenztes Re-Attach mit Backoff gemäß `docs/STATE_MACHINE.md` umgesetzt (`AudioEngineState.kt`: `Listening`/`Retrying` neu, `Suspended` entfernt; `AndroidAudioEngine.kt`: 5 Versuche/2s–30s, Generationszähler gegen überholte Retries). Punkt 4 galt als so weit abgeschlossen wie ohne weitere Geräte möglich (Spotify informell bestätigt, SoundCloud/YouTube als Kooperationsproblem dokumentiert, nicht durch Re-Attach lösbar – Re-Attach hilft nur bei Playern, die überhaupt eine Session melden). **Nicht auf echter Hardware verifiziert** – CI (Build/Lint/Unit-Tests) ist die einzige verfügbare Prüfung in dieser Sandbox.
