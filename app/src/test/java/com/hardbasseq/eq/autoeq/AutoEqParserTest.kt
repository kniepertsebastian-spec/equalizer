...
class AutoEqParserTest {
    ...
    fun parseAutoEqText_graphicEqLineFormat_parsedSuccessfully() {
        ...
        val preset = result.getOrThrow()
        assertEquals("Sony WH-1000XM4 AutoEQ", preset.name)
        assertEquals(4, preset.targetCurve.size)
        assertEquals(50f, preset.targetCurve[1].frequencyHz, 0.001f)
        assertEquals(3.5f, preset.targetCurve[1].gainDb, 0.001f)
    }
    ...
    fun parseAutoEqText_tsvFormat_parsedSuccessfully() {
        ...
        val preset = result.getOrThrow()
        assertEquals(4, preset.targetCurve.size)
        assertEquals(60f, preset.targetCurve[1].frequencyHz, 0.001f)
        assertEquals(4.0f, preset.targetCurve[1].gainDb, 0.001f)
    }
}
